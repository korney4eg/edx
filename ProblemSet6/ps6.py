...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, text):
        def findWord(text, word):
            ...
            ind=text.find(word)
            if ind >= 0:
                if (text[ind-1] not in string.letters) and (text[ind+len(word)] not in string.letters):
                    return True
                else:
                    return findWord(text[ind+1:],word)
            else:
                return False

        return findWord(text,self.word)
...
class AndTrigger(Trigger):
    ...
    def evaluate(self,text):
        return self.trigger1.evaluate(text) and self.trigger2.evaluate(text)
...
class OrTrigger(Trigger):
    def __init__(self, trigger1,trigger2):
        ...
        self.trigger2 = trigger2;
    def evaluate(self,text):
        return self.trigger1.evaluate(text) or self.trigger2.evaluate(text)
...
def makeTrigger(triggerMap , triggerType, params, name):
    ...
    (ex: ["world"]) name: a string representing the name of the new trigger
    (ex: "t1")
